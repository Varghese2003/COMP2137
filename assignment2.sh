...
sudoaccess() {
    if ! sudo grep -qx "dennis ALL=(ALL) NOPASSWD:ALL" /etc/sudoers; then
        echo "Adding sudo access for user dennis"
        echo "dennis ALL=(ALL) NOPASSWD:ALL" | sudo tee -a /etc/sudoers >/dev/null
    else
...
ufwrules(){
    ...
    sudo ufw allow on $mgmtinterface port 22
    sudo ufw allow port 80
    sudo ufw allow port 3128
